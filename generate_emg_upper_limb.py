...
Mac compatibility
-----------------
* Device auto-selection: CUDA (Linux/Windows GPU) > MPS (Apple Silicon) > CPU.
...
def auto_select_device() -> str:
    ...
    if platform.system() == "Darwin":
        # Apple OS: MPS if available, else CPU (never try CUDA on Mac)
    ...
    if torch.cuda.is_available():
        ...
        print(f"[info] CUDA GPU detected: {name}")
        return "cuda"
    return "cpu"
...
def sample_static_muaps(generator, properties, num_mus, latent_dim, device, b, a,
    ...
    ``chunk_size`` caps the number of MUs sent to BioMime at once so the GPU
    does not run out of memory for large pools (e.g. BRA=800 MUs).
    """
    ...
    cond = torch.cat([num, depth, angle, iz, cv, length], dim=1)  # keep on CPU for chunking

    chunks = []
    with torch.no_grad():
        for start in range(0, num_mus, chunk_size):
            end = min(start + chunk_size, num_mus)
            n = end - start
            cond_chunk = cond[start:end].to(device)
            zi_chunk = torch.randn(n, latent_dim, device=device)
            sim_chunk = generator.sample(n, cond_chunk, device, zi_chunk)  # [n, 10, 32, 96]
            chunks.append(sim_chunk.permute(0, 2, 3, 1).detach().cpu())
            del sim_chunk, cond_chunk, zi_chunk
            if device.type == "cuda":
                torch.cuda.empty_cache()
    sim = torch.cat(chunks, dim=0).numpy()  # [num_mus, 10, 32, 96]
...
def main():
    ...
    ap.add_argument("--device", type=str, default="auto",
                    ...
                    help=("Compute device. 'auto' (default): CUDA > MPS > CPU. "
    ...
    # Device selection: auto-detect or validate user override
    if args.device == "auto":
    ...
    # save per-muscle npz for ALL muscles
    for ms in UPPER_LIMB_MS:
    ...
    print(f"[info] saved per-muscle npz for {len(UPPER_LIMB_MS)} muscles: {', '.join(UPPER_LIMB_MS)}")
    ...
    # plot activation envelopes vs |EMG| for ALL muscles
    n_ms = len(UPPER_LIMB_MS)
    fig, axes = plt.subplots(n_ms, 1, figsize=(12, 2.2 * n_ms), sharex=True)
    win = max(1, int(0.05 * args.fs_out))  # 50-ms RMS window
    for ax, ms in zip(axes, UPPER_LIMB_MS):
        emg_ms = per_muscle_emg[ms]
        sig = emg_ms[5, 16, :time_samples]          # centre of grid
        env = np.convolve((np.abs(sig))**2, np.ones(win)/win, mode="same") ** 0.5
        ax2 = ax.twinx()
        ...
        ax.set_ylabel(f"{ms}\nact", color="tab:orange", fontsize=8)
        ax2.set_ylabel("EMG", color="tab:blue", fontsize=7)
        ax.tick_params(axis="y", labelsize=6)
        ax2.tick_params(axis="y", labelsize=6)
    axes[-1].set_xlabel("time (s)")
    fig.suptitle("Activation (orange) vs synthetic EMG envelope (blue) — all muscles")
    fig.tight_layout()
